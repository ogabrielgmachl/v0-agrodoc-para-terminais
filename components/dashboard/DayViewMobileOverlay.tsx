...
export function DayViewMobileOverlay({ 
  ...
  // Daily Averages (Arithmetic and Weighted)
  const dailyAverages = useMemo(() => {
    const trucksWithData = completeTrucks.filter(t => 
      t.cor !== null || t.pol !== null || t.umi !== null || t.cin !== null || t.ri !== null
    )
    
    const calculateAverage = (metric: 'cor' | 'pol' | 'umi' | 'cin' | 'ri') => {
      const trucksWithMetric = trucksWithData.filter(t => t[metric] !== null)
      if (trucksWithMetric.length === 0) return { arithmetic: null, weighted: null }
      
      // Arithmetic average
      const sum = trucksWithMetric.reduce((acc, t) => acc + (t[metric] || 0), 0)
      const arithmetic = sum / trucksWithMetric.length
      
      // Weighted average (by weight)
      const totalWeight = trucksWithMetric.reduce((acc, t) => acc + (t.grossWeight || 0), 0)
      if (totalWeight === 0) return { arithmetic, weighted: null }
      
      const weightedSum = trucksWithMetric.reduce((acc, t) => 
        acc + ((t[metric] || 0) * (t.grossWeight || 0)), 0
      )
      const weighted = weightedSum / totalWeight
      
      return { arithmetic, weighted }
    }
    
    return {
      cor: calculateAverage('cor'),
      pol: calculateAverage('pol'),
      umi: calculateAverage('umi'),
      cin: calculateAverage('cin'),
      ri: calculateAverage('ri'),
    }
  }, [completeTrucks])
    ...
    <div className={`fixed inset-0 z-[9997] flex flex-col ${isDarkMode ? "bg-[#0a0f1a]" : "bg-slate-50"}`}>
      ...
      <div className="flex-1 overflow-auto px-4 pb-4">
        {viewMode === "list" ? (
          ...
          /* Table view - Mobile with horizontal scroll */
          <div className="overflow-x-auto -mx-4">
            <table className="w-full min-w-[800px]">
              <thead className={isDarkMode ? "bg-slate-800/50" : "bg-slate-100"}>
                <tr>
                  <th className={`px-3 py-2 text-left text-[10px] font-bold uppercase ${isDarkMode ? "text-slate-400" : "text-slate-600"}`}>Placa</th>
                  <th className={`px-3 py-2 text-left text-[10px] font-bold uppercase ${isDarkMode ? "text-slate-400" : "text-slate-600"}`}>NF</th>
                  <th className={`px-3 py-2 text-left text-[10px] font-bold uppercase ${isDarkMode ? "text-slate-400" : "text-slate-600"}`}>Cliente</th>
                  <th className={`px-3 py-2 text-left text-[10px] font-bold uppercase ${isDarkMode ? "text-slate-400" : "text-slate-600"}`}>Fornecedor</th>
                  <th className={`px-3 py-2 text-right text-[10px] font-bold uppercase ${isDarkMode ? "text-slate-400" : "text-slate-600"}`}>Peso (T)</th>
                  <th className={`px-3 py-2 text-center text-[10px] font-bold uppercase ${isDarkMode ? "text-slate-400" : "text-slate-600"}`}>Status</th>
                  <th className={`px-3 py-2 text-center text-[10px] font-bold uppercase ${isDarkMode ? "text-slate-400" : "text-slate-600"}`}>Ações</th>
                </tr>
              </thead>
              <tbody className={isDarkMode ? "divide-y divide-slate-700/30" : "divide-y divide-slate-200"}>
                {filteredTrucks.length === 0 ? (
                  <tr>
                    <td colSpan={7} className={`px-4 py-12 text-center text-sm ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>
                      {showOnlyIncomplete ? "Todos os caminhões já possuem análises." : "Nenhum caminhão encontrado"}
                    </td>
                  </tr>
                ) : (
                  filteredTrucks.map((truck) => {
                    ...
                    return (
                      <tr 
                        key={truck.id}
                        className={`${isDarkMode ? "hover:bg-slate-800/30" : "hover:bg-slate-50"} transition-colors`}
                      >
                        <td className={`px-3 py-3 text-xs font-mono font-semibold ${isDarkMode ? "text-white" : "text-slate-900"}`}>
                          {truck.licensePlate || "-"}
                        </td>
                        <td className={`px-3 py-3 text-xs ${isDarkMode ? "text-slate-300" : "text-slate-700"}`}>
                          {truck.nf || "-"}
                        </td>
                        <td className={`px-3 py-3 text-xs ${isDarkMode ? "text-slate-300" : "text-slate-700"}`}>
                          {getShortName(truck.client)}
                        ...
                        <td className={`px-3 py-3 text-xs ${isDarkMode ? "text-slate-300" : "text-slate-700"}`}>
                          {getShortName(truck.provider)}
                        </td>
                        <td className={`px-3 py-3 text-xs text-right font-medium ${isDarkMode ? "text-slate-300" : "text-slate-700"}`}>
                        ...
                        <td className="px-3 py-3">
                          {isIncomplete ? (
                            ...
                            <span className={`inline-flex px-2 py-1 rounded-full text-[10px] font-medium ${statusColor}`}>
      ...
      {/* Quality Snapshot - Mobile (Médias do Dia) */}
                ...
                {dailyAverages.cor.arithmetic !== null && (() => {
                  ...
                  <div className={`p-3 rounded-lg border ${isDarkMode ? "bg-slate-800/50 border-slate-700" : "bg-slate-50 border-slate-200"}`}>
                    ...
                    <div className="space-y-1.5">
                      <div>
                        <div className={`text-[10px] ${isDarkMode ? "text-slate-500" : "text-slate-400"}`}>Aritmética</div>
                      ...
                      {dailyAverages.cor.weighted !== null && (
                        <div>
                          <div className={`text-[10px] ${isDarkMode ? "text-slate-500" : "text-slate-400"}`}>Ponderada</div>
